...
$$P_{motor\_next} = \min\left(P_{motor\_limit},\ P_{motor\_current} + \text{MOTOR\_RAMP\_RATE} \cdot dt_{sub}\right)$$
*   $P_{motor\_limit}$: User-configured motor power limit ($\text{W}$)
...
*   $R_{gen\_int}$: Generator internal resistance $= 0.4\ \Omega$ (constant `GENERATOR_INTERNAL_RESISTANCE`)

#### 4.3 Electromagnetic Braking Force ($F_{brake}$)
$$F_{brake} = K_t \cdot G \cdot I_a \cdot \text{sign}(v)$$
*   $K_t$: Torque constant $= 0.15\text{ N}\cdot\text{m/A}$ (assuming ideal coupling where $K_t = K_e$)

#### 4.4 Pulley & Cable Friction ($F_{friction}$)
...
*   Generated Power ($P_{gen\_real}$):
    $$P_{gen\_real} = F_{brake} \cdot |v| \cdot \eta_{gen}$$
*   Load Power ($P_{load}$):
...
$$F_{net\_upward} = -m \cdot g + F_{brake} + |F_{drag}| + F_{friction}$$
$$a = \frac{F_{net\_upward}}{m}$$
...
*   Updated Compaction: $C_{soil\_next} = \min(100,\ C_{soil} + \Delta C_{soil})$
...
#### 6.2 Battery Internal Resistance ($R_{batt\_int}$)
*   24V System: $R_{batt\_int} = 0.04\ \Omega$
*   48V System: $R_{batt\_int} = 0.08\ \Omega$

#### 6.3 Terminal Voltage ($V_{terminal}$)
$$V_{terminal} = \max\left(0,\ V_{oc} + I_{batt} \cdot R_{batt\_int}\right)$$
...
*   Updated Temperature:
    $$T_{batt\_next} = \max\left(25.0,\ T_{batt} + \frac{dT_{batt}}{dt} \cdot dt_{sub}\right)$$
